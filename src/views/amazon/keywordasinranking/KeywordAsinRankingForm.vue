...
/** 关键词排名任务结果 表单 */
defineOptions({ name: 'KeywordAsinRankingForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await KeywordAsinRankingApi.getKeywordAsinRanking(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as KeywordAsinRanking
    if (formType.value === 'create') {
      await KeywordAsinRankingApi.createKeywordAsinRanking(data)
      message.success(t('common.createSuccess'))
    } else {
      await KeywordAsinRankingApi.updateKeywordAsinRanking(data)
